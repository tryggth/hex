...
def draw_minimax(ax, G, root, leaves, pos):
    """Draw the massive gray Minimax tree."""
    # Edges
    segments = []
    for u, v in G.edges():
        segments.append([pos[u], pos[v]])
    lc = LineCollection(segments, colors=GRAY_EDGE, linewidths=0.5, alpha=0.55)
    ...
    if internal:
        ...
        ax.scatter(xs, ys, s=4, c=GRAY_NODE, alpha=0.6, zorder=3, edgecolors="none")

    # Leaf nodes — slightly lighter
    if leaf_list:
        xs = [pos[n][0] for n in leaf_list]
        ys = [pos[n][1] for n in leaf_list]
        ax.scatter(xs, ys, s=3, c=GRAY_LEAF, alpha=0.45, zorder=3, edgecolors="none")

    # Root node — slightly larger
    rx, ry = pos[root]
    ax.scatter([rx], [ry], s=30, c=GRAY_NODE, alpha=0.85, zorder=4, edgecolors="none")
...
def draw_mcts(ax, G, root, pos, primary_nodes, primary_edges, labels):
    ...
    if non_path_segs:
        lc = LineCollection(non_path_segs, colors=GRAY_EDGE, linewidths=0.8, alpha=0.35)
        ax.add_collection(lc)
    ...
    if non_path:
        ...
        ax.scatter(xs, ys, s=25, c=GRAY_NODE, alpha=0.45, zorder=3, edgecolors="none")
...
def main():
    ...
    # ── Create figure ──
    # Use dpi=100 with figsize in inches to get 3840×2160 px output.
    # Font sizes are in points and render correctly at standard DPI.
    DPI = 100
    ...
    # ── Divider line ──
    ax.plot(
        [1920, 1920], [Y_BOTTOM - 50, Y_TOP + 80],
    ...
    # ── Save ──
    output = "combinatorial_explosion.png"
